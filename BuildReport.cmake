...
function(print_build_report)
	...
	colormsg(HIMAG "C" HIWHITE "No-Opt:         "  WHITE "${CMAKE_C_FLAGS}" HIBLUE "${OPT_CFLAGS_SPC}")
	colormsg(HIMAG "C" HIWHITE "Optimized:      " WHITE "${CMAKE_C_FLAGS}" HIBLUE "${NO_OPT_CFLAGS_SPC}")
	colormsg("")
	colormsg(GREEN "CXX" HIWHITE "No-Opt:       " WHITE "${CMAKE_CXX_FLAGS}" HIBLUE "${OPT_CXXFLAGS_SPC}")
	colormsg(GREEN "CXX" HIWHITE "Optimized:    " WHITE "${CMAKE_CXX_FLAGS}" HIBLUE "${NO_OPT_CXXFLAGS_SPC}")
	colormsg("")
	colormsg(HIRED "Fortran" HIWHITE "No-Opt:   " WHITE "${CMAKE_Fortran_FLAGS}" HIBLUE "${OPT_FFLAGS_SPC}")
	colormsg(HIRED "Fortran" HIWHITE "Optimized:" WHITE "${CMAKE_Fortran_FLAGS}" HIBLUE "${NO_OPT_FFLAGS_SPC}")
	...
	if(DEFINED MPI)
	color_print_bool("MPI parallel version:   " ${MPI})
	endif()
	
	if(DEFINED OPENMP)
	color_print_bool("OpenMP parallel version:" ${OPENMP})
	endif()
	
	if(DEFINED CUDA)
	color_print_bool("CUDA version:           " ${CUDA})
	endif()
	...
	colormsg(" -Python Interpreter:   " HIBLUE "Internal Miniconda" YELLOW "(version ${PYTHON_VERSION})")
	else()
	colormsg(" -Python Interpreter:   " HIBLUE "${PYTHON_EXECUTABLE}" YELLOW "(version ${PYTHON_VERSION})")
	endif()
	endif()
